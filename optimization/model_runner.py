...
class ModelRunner:
    def __init__(self, optimize_input, optimize_output, config: Config):
    ...
    def process_results(self):
        ...
        for i in range(0, len(self.opt_result.slack)):
            ...
            if slack < 1.0 and marginal != 0:
                ...
                gain_items = ""
                count_terms = 0
                for ii in range(0, self.config.num_items):
    ...
    def display_conversions(self):
        print("\n----- Conversions -------")
        conversion_data = []
        for trade in self.free_item_conversions:
    ...
    def display_optimization_results(self):
        ...
        print(f"{self.optimize_for.name} per Week:   {-1.0 / self.opt_result.fun:10.5f}")
        print(f"{self.optimize_for.name} per {self.optimize_input.name}: {items_per_input:10.5f}")
        print(f"{self.optimize_input.name} per {self.optimize_for.name}: {1.0 / items_per_input:10.5f}")
