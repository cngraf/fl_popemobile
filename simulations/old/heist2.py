...
class Prize(HeistCard):
    ...
    def can_draw(self, state: HeistState):
        req_progress = 5
        if state.heist_target == HeistLocation.BASEBORN_FOWLINGPIECE:
            req_progress = 7

        return state.items[Item.BurglarsProgress] >= req_progress
...
print(f"Completed {runs} runs with {info} Info & {keys} Keys at {target}")
